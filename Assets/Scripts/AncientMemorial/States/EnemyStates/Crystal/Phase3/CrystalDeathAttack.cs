...
namespace AncientMemorial.States.EnemyStates.Crystal.Phase3 {
	public class CrystalDeathAttack : CrystalPhase3Attack {
		...
		// 정적 프로퍼티
		private static readonly int CRYSTAL_ULT_EFFECT = "CrystalUltEffect".GetHash();
		private static readonly int SLASH_EFFECT = "SlashEffect".GetHash();
		private static readonly int CRYSTAL_DEATH_ATTACK = "crystalDeathAttack".GetHash();
		...
		public override void OnEnter() {
			...
			crystal.PlaySFX(CRYSTAL_DEATH_ATTACK);
			crystal.To<CrystalPhase3>().StopGimmick();
			
			attackEffect = UUI.Get(CRYSTAL_ULT_EFFECT, GameManager.instance.mainScreenCanvas);
		}
		...
		public override void OnRoutine() {
			if (step == 0 && isProgress(0.95f)) {
				...
				GameObject eff1 = UObject.Get(SLASH_EFFECT, crystal.transform.position, PlayEffect: false);
				...
				GameObject eff2 = UObject.Get(SLASH_EFFECT, crystal.transform.position, PlayEffect: false);
				...
				CameraManager.instance.ShakeLerp(75, 10f);
