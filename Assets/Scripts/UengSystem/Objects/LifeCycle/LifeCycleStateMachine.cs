...
namespace UengSystem.Objects.LifeCycle {
	public sealed class LifeCycleStateMachine {
		...
		private void CheckConfiguration(LifeCycleState State) {
			...
			if (isPrepared)
				throw new InvalidOperationException(
					"Life cycle states can only be configured before OnFirstGet completed."
				);
		...
		public void SetDefaultStates(
			...
			// OnFirstGet 전에 실행하도록 강제함
			if (isPrepared)
				throw new InvalidOperationException("Life cycle is already prepared.");

			if (GettingState != null) {
				CheckConfiguration(GettingState);
				this.GettingState ??= GettingState;
			}

			if (ReleasingState != null) {
		...
		public void Prepare() {
			...
			SetDefaultStates(
				new DefaultGetting(target),
				new DefaultReleasing(target)
			);
		...
		public void Get(bool PlayEffect) {
			if (
				...
				throw new InvalidOperationException(
					"Only prepared, released UObject can begin new life."
				);
		...
		public void Release(bool PlayEffect = true) {
			...
			if (UObject.isStoppingLifeCycles) {
				Shutdown();
				return;
			}
		...
		public bool IsCurrent(LifeCycleState State, long Execution) {
			...
			bool isRightPhase =
				phase is LifeCyclePhase.Getting or LifeCyclePhase.Releasing;
		...
		public void Tick(long Execution, float DeltaTime) {
			if (
				...
				Starting ||
				Entering ||
				!IsCurrent(currentState, Execution)
			)
				return;
		...
		public void Shutdown() {
			...
			if (phase != LifeCyclePhase.Released)
				phase = LifeCyclePhase.Releasing;
			...
			State?.Exit();
			target.StopLife(ReleaseStarted);
