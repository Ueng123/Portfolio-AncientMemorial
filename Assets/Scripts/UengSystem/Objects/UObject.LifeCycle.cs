...
namespace UengSystem.Objects {
	public partial class UObject {
		...
		public bool canStartOwnedWork =>
			!isStoppingLifeCycles &&
			!StoppingRunningTask &&
			!lifeCycle.isShuttingDown &&
			(!lifeCycle.isPrepared || !isReleased);
		...
		internal static void CheckAcquireAllowed() {
			if (isStoppingLifeCycles)
				throw new InvalidOperationException(
					"Cannot acquire objects during scene shutdown."
				);
		}
		...
		public static void ShutdownLifeCycles() {
			...
			foreach (UObject Target in PreparedObjects.ToArray()) {
				if (Target)
		...
		public static void BeginSceneLifeCycles() =>
			isStoppingLifeCycles = false;
		...
		private static void RunActive(Action<UObject> Callback) {
			...
			for (int Index = 0; Index < InitialCount; Index++) {
				...
				Callback.Invoke(Entry.Target);
			}
		}

		public static void UpdateRoutine() {
			GameManager.instance.currentUpdatePhase =
				UpdateRoutineType.Processing;

			RunActive(
				Target =>
					Target.ExecuteProcesses(Target.ProcessToUpdate)
			);

			GameManager.instance.currentUpdatePhase =
				UpdateRoutineType.EarlyRoutine;

			RunActive(Target => Target.EarlyRoutine());

			GameManager.instance.currentUpdatePhase =
				UpdateRoutineType.Routine;
		...
		public static void LifeCycleRoutine() {
			...
			if (GameManager.instance)
				GameManager.instance.currentUpdatePhase =
					UpdateRoutineType.LifeCycleRoutine;
			...
			for (int Index = 0; Index < InitialCount; Index++) {
				...
				Entry.Target.lifeCycle.Tick(
					Entry.Execution,
					Entry.Target.lifeCycleDeltaTime
				);
			}
			
			if (GameManager.instance)
				GameManager.instance.currentUpdatePhase =
					UpdateRoutineType.RoutineEnd;
		}

		public static void FixedUpdateRoutine() {
			GameManager.instance.currentFixedUpdatePhase =
				FixedUpdateRoutineType.Processing;

			RunActive(
				Target =>
					Target.ExecuteProcesses(Target.ProcessToFixedUpdate)
			);

			GameManager.instance.currentFixedUpdatePhase =
				FixedUpdateRoutineType.FixedRoutine;

			RunActive(Target => Target.FixedRoutine());

			GameManager.instance.currentFixedUpdatePhase =
				FixedUpdateRoutineType.RoutineEnd;
		}

		// 인스턴스 메서드
		public void SetGettingState(Getting State) =>
			lifeCycle.SetGettingState(State);

		public void SetReleasingState(Releasing State) =>
			lifeCycle.SetReleasingState(State);

		public void SetStates(
			Getting GettingState,
			Releasing ReleasingState
		) =>
			lifeCycle.SetStates(GettingState, ReleasingState);
		
		protected void SetDefaultStates(
			Getting GettingState = null,
			Releasing ReleasingState = null
		) =>
			lifeCycle.SetDefaultStates(
				GettingState,
				ReleasingState
			);
		...
		public virtual void OnFirstGet() {
			if (FirstGetCompleted)
				throw new InvalidOperationException(
					"OnFirstGet must run only once per instance."
				);
			...
			if (whenInitialize != WhenInitialize.Never)
		...
		public void SetPoolReturnMethod(Action<UObject> Return) {
			if (PoolReturn != null || !isReleased)
				throw new InvalidOperationException(
					"Pool ownership is immutable."
				);

			PoolReturn = Return;
		}

		protected void BeginLife(
			bool PlayEffect,
			Action<UObject> Configure = null
		) {
			CheckAcquireAllowed();
		...
		public void Get(bool PlayEffect = true) {
			if (PoolReturn != null)
				throw new InvalidOperationException(
					"Acquire pooled objects through static Get."
				);
		...
		public void PrepareGetting() {
			...
			if (
				lifeNumber != Life ||
				lifeCycle.phase != LifeCyclePhase.Getting
			)
				return;
		...
		public void RegisterLifeCycle(long Execution) {
			if (LifeEntry != null)
				throw new InvalidOperationException(
					"Duplicate life cycle registration."
				);

			LifeEntry =
				new RoutineEntry(this, Execution);
		...
		public void StopLife(bool ReleaseStarted) {
			...
			if (
				!ReleaseStarted &&
				lifeCycle.phase != LifeCyclePhase.Released
			)
				OnRelease();
		...
		private void SetLifeColliders(bool Enabled) {
			for (
				int Index = 0;
				Index < LifeColliders.Length;
				Index++
			) {
				if (LifeColliders[Index])
					LifeColliders[Index].enabled =
						Enabled &&
						ColliderEnabled[Index];
			}
		}
