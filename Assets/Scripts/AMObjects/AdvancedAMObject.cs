...
namespace AncientMemorial.AMObjects {
	public abstract class AdvancedAMObject : AMObject {

		public static BufferedList<AdvancedAMObject> AdvancedInstances = new ();
		...
		public static void UpdateRoutine() {
			for (int i = 0; i < updatePhaseCount; i++) {
				...
				foreach (AdvancedAMObject ins in AdvancedInstances) {
		...
		public static void EventRoutine() {
			List<Event> events = EventManager.instance.GetEvents();
